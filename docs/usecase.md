...
Version 0.5.5 updates public names to `github.com/criad-com` while retaining the flattened example, editable derived layers and stock USD render under MIT. The two study renders and clipped shells retain the preceding release's presentation and geometry. The nine APIs and 71 properties retain the v0.5.0 contract. Toolchain v0.3.8 checks the public org name alongside S29 source portability, publication freshness, plugin-free rendering and MIT; the earlier results remain in [migration acceptance](migration.md). Native compute has a separate release and evidence boundary.
